...
Module.register("MMM-NsInfo", {
	...
	getDom: function () {
		...
		if (this.config.apiKey === null || this.config.apiKey === "" || this.config.apiKey === "YOUR_API_KEY_HERE") {
			wrapper.innerHTML = "Please set the an NS <i>API_KEY</i> in the config for module: " + this.name + ".";
		...
		if (this.travels.length === 0) {
			wrapper.innerHTML = "Could not load departures, please check the console for more details.";
			wrapper.className = "dimmed light small";
			return wrapper;
		}
		...
		for (var t in this.travels) {
			...
			timeCell.innerHTML = travel.actualDepartureDate.split("T")[1].split("+")[0].split(":", 2).join(":");
	...
	getHeader: function () {
		if (this.config.header === null) {
			return this.config.station ? "reizen vanaf " + this.config.station : "NS Reizen";
		} else {
	...
	updateTravels: function () {
		...
		travelRequest.setRequestHeader("Ocp-Apim-Subscription-Key", this.config.apiKey);
	...
	getParams: function () {
		...
		if (this.config.lang && this.config.stationId) {
			params += "lang=" + this.config.lang + "&station=" + this.config.stationId + "&maxJourneys=" + this.config.maxDepartures;
		} else {
	...
	processTravel: function (data) {
		...
		for (var i = 0, count = travelList.length; i < count; i++) {
			...
			const calculatedDelay = (parsedActualDate - parsedPlannedDate) / (1000 * 60);
			let travelData = {
